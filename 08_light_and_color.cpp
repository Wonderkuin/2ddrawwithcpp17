...
void read_image(std::vector<std::tuple<float, float, float>> &image, int &width, int &height, std::string filename)
{
    ...
    while (getline(infile, line))
    {
        ...
        if (!(line.compare("255") == 0) && state == 3)
        {
            ...
            wcolor = std::make_tuple(float(std::get<0>(rcolor) / 255.0f), \
                                        float(std::get<1>(rcolor) / 255.0f), \
                                        float(std::get<2>(rcolor) / 255.0f));
...
void save_image(std::vector<std::tuple<float, float, float>> &image, int width, int height, std::string filename)
{
    ...
    out << "P3\n" << width << " " << height << "\n255\n";
    for (int i = 0; i < calc_size(width, height); ++i)
    {
        color = image[i];
...
void negative_image(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            image[get_index(x, y, width)] = std::make_tuple(1.0f - std::get<0>(oldColor), \
                                                            1.0f - std::get<1>(oldColor), \
                                                            1.0f - std::get<2>(oldColor)\
                                                            );
        }
    }
}
...
void double_as_dark(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            oldColor = image[get_index(x, y, width)];
            image[get_index(x, y, width)] = std::make_tuple(std::get<0>(oldColor) / 2.0f, \
                                                            std::get<1>(oldColor) / 2.0f, \
                                                            std::get<2>(oldColor) / 2.0f\
                                                            );
        }
    }
}
...
void onefive_as_dark(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            oldColor = image[get_index(x, y, width)];
            image[get_index(x, y, width)] = std::make_tuple(std::get<0>(oldColor) / 1.5f, \
                                                            std::get<1>(oldColor) / 1.5f, \
                                                            std::get<2>(oldColor) / 1.5f\
                                                            );
        }
    }
}
...
void twice_as_bright(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            oldColor = image[get_index(x, y, width)];
            r = std::get<0>(oldColor) * 2.0f;
            g = std::get<1>(oldColor) * 2.0f;
            b = std::get<2>(oldColor) * 2.0f;
            if (r > 1.0) { r = 1.0f; }
            if (g > 1.0) { g = 1.0f; }
            if (b > 1.0) { b = 1.0f; }
...
void add_for_brighter(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            oldColor = image[get_index(x, y, width)];
            r = std::get<0>(oldColor) + 0.2f;
            g = std::get<1>(oldColor) + 0.2f;
            b = std::get<2>(oldColor) + 0.2f;
            if (r > 1.0) { r = 1.0f; }
            if (g > 1.0) { g = 1.0f; }
            if (b > 1.0) { b = 1.0f; }
            image[get_index(x, y, width)] = std::make_tuple(r, g, b);
...
void greyscale_image(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            oldColor = image[get_index(x, y, width)];
            c = 0.2126 * std::get<0>(oldColor) + \
                0.7152 * std::get<1>(oldColor) + \
                0.0722 * std::get<2>(oldColor);
            if (c > 1.0) { c = 1.0f; }
...
void r_zero_b_image(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            color = image[get_index(x, y, width)];
            image[get_index(x, y, width)] = std::make_tuple(std::get<0>(color), 0.0, std::get<2>(color));
        }
    }
}
...
void rg_zero_image(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            color = image[get_index(x, y, width)];
            image[get_index(x, y, width)] = std::make_tuple(std::get<0>(color), std::get<1>(color), 0.0);
        }
    }
}
...
void grb_image(std::vector<std::tuple<float, float, float>> &image, int width, int height)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        {
            color = image[get_index(x, y, width)];
            image[get_index(x, y, width)] = std::make_tuple(std::get<1>(color), std::get<0>(color), std::get<1>(color));
        }
    }
}
...
void blended_two_imaged(std::vector<std::tuple<float, float, float>> &blend1, \
...
int main()
{
    ...
    std::cout << "Saving: '08_double_as_dard_image.ppm'." << std::endl;
    save_image(image, width, height, "08_double_as_dard_image.ppm");
    ...
    std::cout << "Saving: '08_onefive_as_dard_image.ppm'." << std::endl;
    save_image(image, width, height, "08_onefive_as_dard_image.ppm");
    ...
    std::cout << "Saving: '08_rrb_image.ppm'." << std::endl;
    ...
    blended_two_imaged(blend1, blend2, width, height, 0.5);
    ...
    blended_two_imaged(blend1, blend2, width, height, 0.2);
