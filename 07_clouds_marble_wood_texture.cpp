...
void generate_marble(std::vector<std::tuple<float, float, float>> &image, int width, int height, std::vector<double> &noise)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            xyValue = x * xPeriod / width + y * yPeriod / height + \
                        turbulencePower * turbulence(double(x), double(y), width, height, turbulenceSize, noise) / 255.0;
