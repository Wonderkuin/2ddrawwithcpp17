...
void draw_line(std::vector<std::tuple<float, float, float>> &image, std::tuple<int, int, int, int> &coords, \
    std::tuple<float, float, float> &color, int width, int height
)
{
...
bool clip_line(std::tuple<int, int, int, int> &coords1, std::tuple<int, int, int, int> &coords2, int width, int height)
{
    ...
    int x2 = std::get<2>(coords1);
    int y2 = std::get<3>(coords1);
...
int main()
{
    ...
    for (int x = 0; x < width; ++x)
    {
        y = 639 - (319.0 - (319.0 * std::pow(1.0 - (1.0 / 339.0), x)) * sin(2 * M_PI * x * (15.0 / numb_of_samples) - 1.0) + 0);
        if (x == 0)
        {
            ox = x;
            oy = y;
    ...
    for (int x = 0; x < width; ++x)
    {
        y = 319.0 - (339.0 * std::pow(1.0 - (1.0 / 339.0), x));
        if (x == 0)
        {
            ox = x;
            oy = y;
